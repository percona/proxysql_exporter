...
const mySQLGlobalQuery = "SELECT Variable_Name, Variable_Value FROM stats_mysql_global ORDER BY Variable_Name"
...
var mySQLGlobalMetrics = map[string]*metric{
	...
	"Client_Connections_created": {"client_connections_created", prometheus.CounterValue,
		"Number of frontend connections created so far."},
...
const mySQLconnectionPoolQuery = "SELECT hostgroup, srv_host, srv_port, * FROM stats_mysql_connection_pool ORDER BY hostgroup, srv_host, srv_port"
